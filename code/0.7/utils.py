...
def create_env():
	if os.path.isfile(os.getcwd()+'.env'):
		...
		# Add INTERNAL_IP1
		intip = get_internal_addr()
		env_data = ''
	myname = cmd('hostname',False).pop()
	extip  = get_ext_ip()
	intip  = get_internal_addr()
	env_data += ('HOSTNAME=%s\n' % myname)
...
def load_local_vars():
	...
	ext_ip = getenv('EXTERNAL_IP')
	int_ip = getenv('INTERNAL_IP1')
	try:
		server = getenv('CENTRAL_SERVER')
	except:
		env_notice()
		exit()
...
def autodiscover_local():
	internal_addr = getenv('INTERNAL_IP1')
	subnet = '.'.join(internal_addr.split('.')[0:-1])+'.0/24'
	c = 'sudo nmap -p 22 %s' % subnet
...
def add_local_peers_pv2():
	if not os.path.isfile('.env'):
		create_env()
	# Check local network for nodes
	possible_nodes = autodiscover_local()
	for ip,mac in possible_nodes:
		prompt = ('[?] Would you like to add %s [%s] as a Peer? [y/n]: ' % (ip, mac))
		if raw_input(prompt).upper() == 'Y':
			# Try to add it 
			register_commandline(ip, mac)
...
def get_node_names():
	return cmd('ls PoolData/Creds/*.pem',False)
